...
async def submit_delegation(
    ...
    # If Cloudinary returned a public_id in the upload result we may want to save it.
    # Note: save_uploaded_file currently sets public_id on upload but returns URL only;
    # if you'd like to return/record public_id, update save_uploaded_file to return it too.

    res = await db.delegations.insert_one(doc)
    rec = await db.delegations.find_one({'_id': res.inserted_id})
    rec['id'] = str(rec['_id'])
    return {'ok': True, 'delegation_id': rec['id']}


@router.get('/delegation/status')
async def get_delegation_status(db=Depends(get_misc_db)):
