...
const CogentLabsLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/cogentlabs/auth/login`, {
        username: formData.username,
        password: formData.password
      });
      // Store JWT token from backend in both localStorage and cookies
      localStorage.setItem("cogentLabsAuthToken", response.data.token);
      Cookies.set("cogentLabsAuthToken", response.data.token, { 
        expires: 1, // 1 day
      ...
      try {
        localStorage.setItem('cogentLabsLastLogin', new Date().toISOString());
      } catch (e) {
        // ignore storage errors
        console.warn('Could not store cogentLabsLastLogin', e);
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 p-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Sign in to manage registrations</p>
        </div>
...
export default CogentLabsLogin;
