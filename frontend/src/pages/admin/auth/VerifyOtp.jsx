...
const OTPVerification = () => {
  ...
  const generateOTP = async () => {
    try {
      setError("");
      // Call your FastAPI endpoint to generate OTP
    const response = await axios.post(`${API_BASE_URL}/admin/otp/generate`, {}, {
        headers: {
          'adminAuthToken': Cookies.get('adminAuthToken'),
          'masterAuthToken': Cookies.get('masterAuthToken')
        }
      });
      setSuccess("OTP has been generated and sent to your admin email!");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Verify OTP with your FastAPI backend
    const response = await axios.post(`${API_BASE_URL}/admin/otp/verify`, {
        otp: otpValue
      }, {
        headers: {
          'adminAuthToken': Cookies.get('adminAuthToken'),
      ...
      if (response.status === 200) {
        ...
        // Set a secure, site-wide cookie to indicate OTP verification
        Cookies.set("otpVerified", "true", { path: "/", secure: true });
        setTimeout(() => {
          navigate("/fake");
        }, 1000);
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 p-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Enter the OTP sent to your admin email</p>
        </div>
        ...
        >
              ...
              <div className="flex justify-center space-x-3">
                {otp.map((data, index) => (
                  <input
                    ...
                    className="w-12 h-12 bg-gray-700 border border-gray-600 rounded-lg text-center text-white text-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ))}
              </div>
            </div>
          ...
          <div className="mt-6 text-center">
            <button
              ...
              className={`text-sm flex items-center justify-center mx-auto ${resendTimer > 0 || isLoading ? "text-gray-500" : "text-cyan-400 hover:text-cyan-300"}`}
            >
        ...
        <div className="mt-6 text-center text-gray-500 text-sm">
          ...
          <p className="mt-2 text-cyan-400">OTP expires after 60 seconds</p>
        </div>
      </motion.div>
    </div>
  );
