...
const Blogs = () => {
  ...
  const DUMMY_POSTS = [
    { id: 'dummy-1', title: 'Welcome to Taakra Blogs', excerpt: 'Insights and updates from our community. Content will load when the server is available.', author: 'Taakra Team', published_at: new Date().toISOString(), slug: 'welcome' }
  ];
  ...
  const fetchPosts = async () => {
    try {
      ...
      setBlogPosts(Array.isArray(data) ? data : DUMMY_POSTS);
      setError(null);
    } catch (err) {
      setBlogPosts(DUMMY_POSTS);
      setError(null);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="container mx-auto px-4 ">
        ...
        {blogPosts.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-xl text-slate-500">No blog posts available at the moment.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogPosts.map((post) => (
              <div
                key={post.id}
                className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm transition-transform duration-200 hover:shadow-lg hover:border-sky-200 cursor-pointer"
