...
const CogentLabsRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE_URL}/register`, fd, {
  ...
  // Redirect to home if registration form is closed
  const navigate = useNavigate();
  useEffect(() => {
    const checkOpen = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/register/status`);
        if (!res.data.open) {
          toast.error("Registration form is currently closed. Redirecting...");
          navigate('/');
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-blue-900 text-white py-16">
      <div className="max-w-3xl mx-auto bg-gray-800 p-8 rounded-lg border border-gray-700">
        <h2 className="text-2xl font-bold mb-4">Cogent Labs - Mock Interview Registration</h2>
...
export default CogentLabsRegistration;
